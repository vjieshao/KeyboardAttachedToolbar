//
//  UIColor+KeyboardAttachedToolbar.swift
...
extension UIColor {
    ...
    public convenience init?(hexString: String, alpha: Float = 1.0) {
        ...
        if (hex.range(of: "(^[0-9A-Fa-f]{6}$)|(^[0-9A-Fa-f]{3}$)", options: .regularExpression) != nil) {
            ...
            if hex.count == 3 {
                let redHex   = hex.substring(from: 0, to: 1)
                let greenHex = hex.substring(from: 1, to: 2)
                let blueHex  = hex.substring(from: 2, to: 3)
                hex = redHex + redHex + greenHex + greenHex + blueHex + blueHex
            }

            let redHex   = hex.substring(from: 0, to: 2)
            let greenHex = hex.substring(from: 2, to: 4)
            let blueHex  = hex.substring(from: 4, to: 6)
